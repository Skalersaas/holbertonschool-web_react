...
describe('Test suite for notification selcetors', () => {
	...
	it('Tests that getUnreadNotifications returns a list of the message entities within the reducer', () => {
		...
		const unread = getUnreadNotificationsByType(initialState);
		expect(unread.count()).toBe(1);
		expect(unread.toJS()[0]['guid']).toBe('cec84b7a-7be4-4af0-b833-f1485433f66e');
	});
	it('Tests that the selector returns unread urgent notifications when the filter is set', () => {
		const initialState = {
			notifications: fromJS({
				filter: 'URGENT'
			})
		...
		initialState.notifications = notificationReducer(initialState.notifications, fetchAction);
		const unread = getUnreadNotificationsByType(initialState);
		expect(unread.count()).toBe(1);
		expect(unread.toJS()[0]['guid']).toBe('cec84b7a-7be4-4af0-b833-f1485433f66e');
	});
